...
resource "null_resource" "run_ansible_playbook" {
  ...
  provisioner "local-exec" {
    command = "ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -i '${azurerm_public_ip.demopublicip.ip_address},' nginx.yml --extra-vars='ansible_ssh_user=saumya' --extra-vars='ansible_ssh_pass=Saumya123' --become --become-user=root"
  }
}
